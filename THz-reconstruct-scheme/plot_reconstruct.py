...
order_reconstruct = 78
delay_0, delay_1, n_delay = 0., 94274., 100
...
def process_data_of_reconstruction ():
    global dt, w0
    n_pts = 100
    result = np.empty ([n_pts, 2])

    delay_0, delay_1 = 0., 94274.
    d_delay = (delay_1 - delay_0) / (n_pts - 1.)
...
def plot_reconstruct_waveform ():
    ...
    plt.plot (data[:,0], 3.2e-2*(10**data[:,1]-5.8e-5), 'rx', markersize=4)
    plt.show ()
...
def plot_full_spectrum_vs_delay ():
    ...
    plt.imshow ( (10**data - 10**data_o_THz), origin='lower', aspect=1000., extent=(0., 94274., 0, 100),  vmin=-0.00, vmax=0.0007 )
