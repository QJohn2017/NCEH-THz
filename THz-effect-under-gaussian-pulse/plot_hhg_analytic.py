...
# d(w)
def plot_hhg (ax, ylim0=-2.5, ylim1=0., lw=1, color=u'#1f77b4', label=''):
    order_max = 100
    ax.plot(order, spec, lw=lw, c=color, label=label)
...
plot_hhg (ax1, ylim0=-4.5, ylim1=-2, lw=1.5, label='Numerical')

# parameters
with open (dirData+'para_4.dat', 'r') as f:
    l = np.array (f.readline().split(' '), float)
    Ip, w0, E0 = l[0], l[1], l[2]
    l = np.array (f.readline().split(' '), float)
    dt = l[2]
E_cutoff = Ip + 3.17*(E0**2/(4.*w0**2))
order_cutoff = E_cutoff / w0
data = np.loadtxt (dirData+'data_4.dat')
time, Ef, Af, alphaf, dreal, dimag = data[:,0], data[:,1], data[:,2], data[:,3], data[:,4], data[:,5]

# for arbitrary field
order, spec = calc_spec (dreal)
plot_hhg (ax1, ylim0=-4.5, ylim1=-2, lw=0.6, color='r', label='from Eq. (x)')
